...
const SLOW_MS = 1200; // 1.2 ثانية (زِد أو قلّل اللي يناسبك)
...
async function getPage() {
  ...
  const browser = await puppeteer.launch({
    headless: false, // 👈 مهم: عشان تشوف البوت
    executablePath: exe,
  ...
  // نخزن الريفرنس عشان نقفل المتصفح بعد انتهاء الوظيفة
...
export async function executeInImdad(job) {
  ...
  try {
    ...
    try {
      ...
      const envKey    = clinicKey ? `IMDAD_CLINIC_OPTION__${clinicKey}` : '';
      const envOpt    = envKey ? process.env[envKey] : null;
    ...
    const pattern = buildSlotPattern(job);
    console.log('[slot_debug] looking for pattern:', pattern);

    if (!pattern) {
      console.error('[slot_debug] pattern is null (date/time invalid)');
      await snap(p, job, '09_no_pattern');
      throw new Error('slot_pattern_invalid');
    }

   const matchedValue = await p.$$eval(
  'input[name="ss"]',
  (els, wanted) => {
    const el = els.find(e => (e.value || '').includes(wanted));
    if (!el) return null;

    // اضغط على الليبل الذي يحتوي هذا الـ input
